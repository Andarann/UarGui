...
#include <stdio.h>      // _vsnprintf, sscanf, printf
#include <cstdio>
...
#pragma warning (disable: 4996) // 'This function or variable may be unsafe': strcpy, strdup, sprintf, _vsnprintf, sscanf, fopen
#endif
...
// Ideally we would test for only one of those limits at runtime depending on the behavior the _vsnprintf(), but trying to deduct it at compile time sounds like a pandora can of worm.
...
// On some platform _vsnprintf() takes va_list by reference and modifies it.
...
void ImGuiTextBuffer::appendv(const char* fmt, va_list args)
{
    ...
    int len = _vsnprintf(NULL, 0, fmt, args);         // FIXME-OPT: could do a first pass write attempt, likely successful on first pass.
    if (len <= 0)
        return;
